...
const BankDepositRecords: React.FC = () => {
  ...
  const getImageUrl = (filename: string) => {
    const envUrl = import.meta.env.VITE_UPLOAD_URL;
    if (envUrl) return `${envUrl}/${filename}`;
    
    const hostname = window.location.hostname;
    // Production: use api.autoflopro.com subdomain
    if (hostname === 'autoflopro.com' || hostname === 'www.autoflopro.com') {
      return `https://api.autoflopro.com/uploads/${filename}`;
    }
    
    const baseUrl = `${window.location.protocol === 'https:' ? 'https' : 'http'}://${hostname}:5001/uploads`;
    ...
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <PrintButton
            formType="receipts"
            data={{ deposits: filteredDeposits }}
            variant="icon"
            size="medium"
          />
