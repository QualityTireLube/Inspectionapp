...
import { useStaticStickerUpdates } from '../contexts/WebSocketProvider';
...
const ActiveStickers: React.FC = () => {
  ...
  const { subscribeToUpdates } = useStaticStickerUpdates();

  useEffect(() => {
    loadData();
  }, []);

  // Subscribe to real-time static sticker updates
  useEffect(() => {
    const unsubscribe = subscribeToUpdates((update) => {
      console.log('🔄 Real-time static sticker update received:', update);
      
      // Reload data to get the latest stickers
      loadData();
      
      // Show notification based on action
      if (update.action === 'created') {
        setRealtimeMessage(`New static sticker created for VIN: ${update.vin}`);
        setShowRealtimeNotification(true);
      } else if (update.action === 'updated') {
        setRealtimeMessage(`Static sticker updated for VIN: ${update.vin}`);
        setShowRealtimeNotification(true);
      } else if (update.action === 'deleted') {
        setRealtimeMessage('Static sticker deleted');
        setShowRealtimeNotification(true);
      } else if (update.action === 'archived') {
        setRealtimeMessage('Static sticker archived');
        setShowRealtimeNotification(true);
      }
    });
  ...
  return (
    <Container maxWidth="lg">
      <Box sx={{ py: 3 }}>
        ...
        <Snackbar
          open={showRealtimeNotification}
          autoHideDuration={4000}
          onClose={() => setShowRealtimeNotification(false)}
