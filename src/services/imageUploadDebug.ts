...
export const convertHEICToJPEG = async (file: File): Promise<File> => {
  try {
    ...
    debug.error('imageUpload', 'HEIC conversion failed:', error);
...
export const resizeImageTo1080p = async (file: File, quality: number = 0.8, attempt: number = 1): Promise<File> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleError = (error: Error) => {
      cleanup();
      debug.warn('imageUpload', `Image resize attempt ${attempt}/${maxAttempts} failed:`, error.message);
      
      if (attempt < maxAttempts) {
        // Retry with exponential backoff
        const delay = Math.pow(2, attempt - 1) * 1000; // 1s, 2s, 4s
        debug.log('imageUpload', `Retrying image resize in ${delay}ms...`);
        setTimeout(() => {
    ...
    img.onload = () => {
      try {
        ...
        if (width <= maxWidth && height <= maxHeight) {
          debug.log('imageUpload', `Image already optimal size (${width}x${height}), converting to JPEG only`);
          ...
          canvas.toBlob((blob) => {
            ...
            debug.log('imageUpload', `Image converted to JPEG: ${file.name} → ${newFile.name}`);
            resolve(newFile);
          }, 'image/jpeg', quality);
        ...
        if (isLargeImage) {
          debug.log('imageUpload', 'Large image detected, using Safari-optimized processing...');
        }
        ...
        canvas.toBlob((blob) => {
          ...
          debug.log('imageUpload', `Image resized successfully: ${file.name} (${img.width}x${img.height}) → ${newFile.name} (${width}x${height})`);
          resolve(newFile);
        }, 'image/jpeg', quality);
...
export const processImageForUpload = async (file: File, resize1080p: boolean = false, attempt: number = 1): Promise<File> => {
  ...
  try {
    debug.group('imageUpload', `Processing image (attempt ${attempt}/${maxAttempts}): ${file.name}`);
    debug.log('imageUpload', 'File details:', {
      name: file.name,
    ...
    if (isHEICFile(file)) {
      debug.log('imageUpload', 'Converting HEIC to JPEG...');
      try {
...
class ImageUploadDebugger {
  ...
  logImageUploadAttempt(file: File | null, error?: any): ImageUploadDebugInfo {
    ...
    debug.group('imageUpload', 'Safari Image Upload Debug');
    debug.log('imageUpload', 'Browser:', debugInfo.browser);
    debug.log('imageUpload', 'Protocol:', window.location.protocol);
    debug.log('imageUpload', 'Capabilities:', {
    ...
    if (debugInfo.fileDetails) {
      debug.log('imageUpload', 'File Details:', debugInfo.fileDetails);
      
      // Check for common iPhone photo issues
      if (debugInfo.fileDetails.type === 'image/heic' || debugInfo.fileDetails.type === 'image/heif') {
        debug.warn('imageUpload', 'iPhone HEIC format detected - conversion required');
      }
      
      if (debugInfo.fileDetails.size > 25 * 1024 * 1024) {
        debug.warn('imageUpload', 'File size exceeds 25MB limit');
      }
    }
    
    if (error) {
      debug.error('imageUpload', 'Upload Error:', error);
