...
class SafariStorage implements StorageInterface {
  ...
  private detectBestStorage(): void {
    ...
    try {
      ...
      debug.log('storage', 'Using localStorage');
      return;
    } catch (e) {
      debug.warn('storage', 'localStorage blocked:', e);
    }
    ...
    try {
      ...
      debug.log('storage', 'Using sessionStorage (localStorage blocked)');
      return;
    } catch (e) {
      debug.warn('storage', 'sessionStorage also blocked:', e);
    }

    // Use memory storage as last resort
    this.storageType = 'memory';
    debug.warn('storage', 'Using memory storage (both localStorage and sessionStorage blocked)');
  }
