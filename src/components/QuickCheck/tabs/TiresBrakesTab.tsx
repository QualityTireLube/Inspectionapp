...
import GuidedVisuals, { GuidedVisualField } from '../../GuidedVisuals';
...
export const TiresBrakesTab: React.FC<TiresBrakesTabProps> = ({
  ...
  // Ref for guided visuals
  const guidedVisualsRef = React.useRef<any>(null);
  
  // Configure guided visual fields
  const guidedVisualFields: GuidedVisualField[] = (
    enabledFields && enabledFields.length > 0
  ) ? [
    // Minimal for No Check: only undercarriage guided visual, tied to drain plug type
    {
      fieldName: 'undercarriage_photo',
      prompt: 'Take a photo of the undercarriage showing the drain plug area',
      guidedVisualType: 'custom',
      triggerConditions: { dependentField: 'drain_plug_type' },
      uploadType: 'undercarriage_photos'
    }
  ] : [
    // Driver Front tire sequence
    {
      fieldName: 'driver_front_tire_tread',
      prompt: 'Capture the tread condition clearly.',
      guidedVisualType: 'tire_tread',
      triggerConditions: {
        tireDateAdded: true,
      },
      uploadType: 'driver_front',
      sequenceStep: 1,
      nextField: 'driver_front_tire_full'
    },
    {
      fieldName: 'driver_front_tire_full',
      prompt: 'Capture a full side view of the tire.',
      guidedVisualType: 'tire_full',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, never from tire date
      },
      uploadType: 'driver_front',
      sequenceStep: 2
    },
    
    // Passenger Front tire sequence
    {
      ...
      sequenceStep: 1,
      nextField: 'passenger_front_tire_full'
    },
    {
      fieldName: 'passenger_front_tire_full',
      prompt: 'Capture a full side view of the tire.',
      guidedVisualType: 'tire_full',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, never from tire date
    ...
    // Driver Rear tire sequence
    {
      ...
      sequenceStep: 1,
      nextField: 'driver_rear_tire_full'
    },
    {
      fieldName: 'driver_rear_tire_full',
      prompt: 'Capture a full side view of the tire.',
      guidedVisualType: 'tire_full',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, never from tire date
    ...
    // Driver Rear Inner tire sequence (for dually)
    {
      ...
      sequenceStep: 1,
      nextField: 'driver_rear_inner_tire_full'
    },
    {
      fieldName: 'driver_rear_inner_tire_full',
      prompt: 'Capture a full side view of the tire.',
      guidedVisualType: 'tire_full',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, never from tire date
    ...
    // Passenger Rear tire sequence
    {
      ...
      sequenceStep: 1,
      nextField: 'passenger_rear_tire_full'
    },
    {
      fieldName: 'passenger_rear_tire_full',
      prompt: 'Capture a full side view of the tire.',
      guidedVisualType: 'tire_full',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, never from tire date
    ...
    // Passenger Rear Inner tire sequence (for dually)
    {
      ...
      sequenceStep: 1,
      nextField: 'passenger_rear_inner_tire_full'
    },
    {
      fieldName: 'passenger_rear_inner_tire_full',
      prompt: 'Capture a full side view of the tire.',
      guidedVisualType: 'tire_full',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, never from tire date
    ...
    // Spare tire sequence
    {
      ...
      uploadType: 'spare',
      sequenceStep: 1,
      nextField: 'spare_tire_full'
    },
    {
      fieldName: 'spare_tire_full',
      prompt: 'Capture a full side view of the tire.',
      guidedVisualType: 'tire_full',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, never from tire date
    ...
    // Front Brake Rotor and Pad sequence (ROTOR FIRST, then PAD)
    {
      fieldName: 'front_brake_photo_rotor',
      prompt: 'Take a photo of the worst rotor condition',
      guidedVisualType: 'brake_pad',
      triggerConditions: {
        rotorConditionSelected: true
      },
      uploadType: 'front_brakes',
      nextField: 'front_brake_photo_pad',
      sequenceStep: 1
    },
    {
      fieldName: 'front_brake_photo_pad',
      prompt: 'Take a photo of the worst brake pad condition',
      guidedVisualType: 'brake_pad',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, not auto-trigger
      },
      uploadType: 'front_brakes',
      sequenceStep: 2
    },

    // Rear Brake Rotor and Pad sequence (ROTOR FIRST, then PAD)
    {
      fieldName: 'rear_brake_photo_rotor',
      prompt: 'Take a photo of the worst rear rotor condition',
      guidedVisualType: 'brake_pad',
      triggerConditions: {
        rearRotorConditionSelected: true
      },
      uploadType: 'rear_brakes',
      nextField: 'rear_brake_photo_pad',
      sequenceStep: 1
    },
    {
      fieldName: 'rear_brake_photo_pad',
      prompt: 'Take a photo of the worst rear brake pad condition',
      guidedVisualType: 'brake_pad',
      triggerConditions: {
        // Step 2 only triggers via sequence progression, not auto-trigger
      },
    ...
    // Undercarriage photo for drain plug
    {
      fieldName: 'undercarriage_photo',
      prompt: 'Take a photo of the undercarriage showing the drain plug area',
      guidedVisualType: 'custom',
      triggerConditions: {
        dependentField: 'drain_plug_type'
      },
      uploadType: 'undercarriage_photos'
    }
  ];
    ...
    return (
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
          ...
          />
          <FormLabel component="legend" sx={{ flexGrow: 1, fontWeight: 'normal', ml: 1 }}>
            {title}
          </FormLabel>
  ...
  // For No Check: render only the allowed subset when a limited field set is provided
  // Check if this is NoCheck by looking for absence of comprehensive tire/brake fields
  const isLimitedInspection = enabledFields && enabledFields.length > 0 && 
    !enabledFields.includes('tire_tread') && !enabledFields.includes('brake_pads');
  
  if (isLimitedInspection) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box>
            <FormControl component="fieldset">
              ...
              <Stack direction="row" spacing={2} sx={{ mt: 1, flexWrap: 'wrap' }}>
                <Chip label="✅ Good" color="success" variant={form.tire_rotation === 'good' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('tire_rotation', 'good')} sx={{ fontWeight: form.tire_rotation === 'good' ? 'bold' : 'normal', mb: 1 }} />
                <Chip label="🔄 Recommend Tire Rotation" color="warning" variant={form.tire_rotation === 'bad' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('tire_rotation', 'bad')} sx={{ fontWeight: form.tire_rotation === 'bad' ? 'bold' : 'normal', mb: 1 }} />
              </Stack>
            </FormControl>
          </Box>
        )}
        ...
        {isEnabled('drain_plug_type') && (
          <Box>
            <FormControl component="fieldset">
              ...
              <Stack direction="row" spacing={2} sx={{ mt: 1, flexWrap: 'wrap' }}>
                <Chip label="✅ Metal" color="success" variant={form.drain_plug_type === 'metal' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('drain_plug_type', 'metal')} sx={{ fontWeight: form.drain_plug_type === 'metal' ? 'bold' : 'normal', mb: 1 }} />
                <Chip label="⚠️ Plastic" color="warning" variant={form.drain_plug_type === 'plastic' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('drain_plug_type', 'plastic')} sx={{ fontWeight: form.drain_plug_type === 'plastic' ? 'bold' : 'normal', mb: 1 }} />
              </Stack>
            </FormControl>
          </Box>
        )}
          ...
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, marginLeft: 'auto' }}>
                ...
                <NotesField fieldName="undercarriage_photos" fieldLabel="Undercarriage" notes={form.field_notes} onNotesChange={onFieldNotesChange} />
              </Box>
            </Box>
            <ImageFieldRectangle
              label=""
              images={(form.undercarriage_photos || []).filter(photo => photo && !photo.isDeleted).map(photo => getDisplayUrl(photo)).filter(url => url && url.length > 0)}
              onImageUpload={onImageUpload}
              onImageRemove={(index) => onDeleteImage('undercarriage_photos', index)}
              onImageView={() => onImageClick(form.undercarriage_photos || [], 'undercarriage_photos')}
              uploadType="undercarriage_photos"
              disabled={loading}
              multiple={true}
              maxFiles={5}
        ...
        {isEnabled('static_sticker') && (
          <Box>
            <FormControl component="fieldset">
              ...
              <Stack direction="row" spacing={2} sx={{ mt: 1, flexWrap: 'wrap' }}>
                <Chip label="✅ Good" color="success" variant={form.static_sticker === 'good' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('static_sticker', 'good')} sx={{ fontWeight: form.static_sticker === 'good' ? 'bold' : 'normal', mb: 1 }} />
                <Chip label="Not Oil Change" color="warning" variant={form.static_sticker === 'not_oil_change' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('static_sticker', 'not_oil_change')} sx={{ fontWeight: form.static_sticker === 'not_oil_change' ? 'bold' : 'normal', mb: 1 }} />
                <Chip label="❌ Need Sticker" color="error" variant={form.static_sticker === 'need_sticker' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('static_sticker', 'need_sticker')} sx={{ fontWeight: form.static_sticker === 'need_sticker' ? 'bold' : 'normal', mb: 1 }} />
              </Stack>
            </FormControl>
          </Box>
        )}
          ...
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, marginLeft: 'auto' }}>
                ...
                <NotesField fieldName="tire_repair_status" fieldLabel="Tire Repair" notes={form.field_notes} onNotesChange={onFieldNotesChange} />
              </Box>
            </Box>
            <Box sx={{ mb: 2 }}>
              <Stack direction="row" spacing={2} sx={{ mt: 1, flexWrap: 'wrap' }}>
                <Chip label="⚠️ Not a tire repair" color="warning" variant={form.tire_repair_status === 'not_tire_repair' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('tire_repair_status', 'not_tire_repair')} sx={{ fontWeight: form.tire_repair_status === 'not_tire_repair' ? 'bold' : 'normal', mb: 1 }} />
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 2, maxWidth: '400px', mt: 2 }}>
                  {(form.tire_repair_status_photos || []).map((photo, index) => (
                    <Box key={index} sx={{ position: 'relative', aspectRatio: '1', borderRadius: 1, overflow: 'hidden', border: '1px solid #ddd', cursor: 'pointer', '&:hover .delete-button': { opacity: 1 } }}>
                      ...
                      <IconButton className="delete-button" onClick={(e) => { e.stopPropagation(); onDeleteImage('tire_repair_status_photos', index); }} sx={{ position: 'absolute', top: 4, right: 4, bgcolor: 'rgba(0,0,0,0.7)', color: 'white', opacity: 0, transition: 'opacity 0.2s', '&:hover': { bgcolor: 'rgba(0,0,0,0.9)' }, padding: '4px' }} size="small">
                        <CloseIcon sx={{ fontSize: '16px' }} />
                      </IconButton>
                    </Box>
                  ))}
        ...
        {/* Tire Repair Guided Layout when repairable */}
        {isEnabled('tire_repair') && form.tire_repair_status === 'repairable' && (
          <Box>
            <TireRepairLayout
          ...
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, marginLeft: 'auto' }}>
                ...
                <NotesField fieldName="tpms_type" fieldLabel="Check TPMS" notes={form.field_notes} onNotesChange={onFieldNotesChange} />
              </Box>
            </Box>
            <Box sx={{ mb: 2 }}>
              <Stack direction="row" spacing={2} sx={{ mt: 1, flexWrap: 'wrap' }}>
                <Chip label="✅ Not a TPMS Check" color="success" variant={form.tpms_type === 'not_check' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('tpms_type', 'not_check')} sx={{ fontWeight: form.tpms_type === 'not_check' ? 'bold' : 'normal', mb: 1 }} />
                <Chip label="❌ TPMS Check" color="error" variant={form.tpms_type === 'bad_sensor' ? 'filled' : 'outlined'} clickable onClick={() => onRadioChange('tpms_type', 'bad_sensor')} sx={{ fontWeight: form.tpms_type === 'bad_sensor' ? 'bold' : 'normal', mb: 1 }} />
              </Stack>
              {form.tpms_type === 'bad_sensor' && (
                <>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 2, maxWidth: '400px', mt: 2 }}>
                      {(form.tpms_tool_photo || []).map((photo, index) => (
                        <Box key={index} sx={{ position: 'relative', aspectRatio: '1', borderRadius: 1, overflow: 'hidden', border: '1px solid #ddd', cursor: 'pointer', '&:hover .delete-button': { opacity: 1 } }}>
                          ...
                          <IconButton className="delete-button" onClick={(e) => { e.stopPropagation(); setForm(f => ({ ...f, tpms_tool_photo: (f.tpms_tool_photo || []).filter((_, i) => i !== index) })); }} sx={{ position: 'absolute', top: 4, right: 4, bgcolor: 'rgba(0,0,0,0.7)', color: 'white', opacity: 0, transition: 'opacity 0.2s', '&:hover': { bgcolor: 'rgba(0,0,0,0.9)' }, padding: '4px' }} size="small">
                            <CloseIcon sx={{ fontSize: '16px' }} />
                          </IconButton>
                        </Box>
                      ))}
                  ...
                  <TPMSLayout
                    ...
                    tpmsSensorTypes={form.tpms_sensor_types || {}}
                    onTPMSStatusChange={onTPMSStatusChange}
                    onTPMSSensorTypeChange={onTPMSSensorTypeChange}
                    showDually={false}
                    onDuallyToggle={() => {}}
                  />
                </>
        ...
        {isEnabled('notes') && (
          <Box>
            ...
            <TextField fullWidth multiline rows={4} label="Notes" name="notes" value={form.notes} onChange={onChange} />
          </Box>
        )}
      </Box>
    );
  ...
  // Default: render full Tires & Brakes UI
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, marginLeft: 'auto' }}>
            ...
            />
            <NotesField
              fieldName="front_shock_struts"
              fieldLabel="Front Shock/Struts"
              notes={form.field_notes}
              onNotesChange={onFieldNotesChange}
            />
            <IconButton
              size="small"
              onClick={(e) => onInfoClick(e, "Inspect front shock absorbers and struts for leaks, damage, or wear.")}
            >
              <InfoIcon fontSize="small" />
            </IconButton>
          </Box>
        ...
        <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          <Chip
            label="✅ Good"
            clickable
            variant={form.front_shock_struts === 'good' ? 'filled' : 'outlined'}
            color={form.front_shock_struts === 'good' ? 'success' : 'default'}
            onClick={() => onRadioChange('front_shock_struts', 'good')}
          />
          <Chip
            label="⚠️ Warning"
            clickable
            variant={form.front_shock_struts === 'warning' ? 'filled' : 'outlined'}
            color={form.front_shock_struts === 'warning' ? 'warning' : 'default'}
            onClick={() => onRadioChange('front_shock_struts', 'warning')}
          />
          <Chip
            label="❌ Bad"
            clickable
            variant={form.front_shock_struts === 'bad' ? 'filled' : 'outlined'}
            color={form.front_shock_struts === 'bad' ? 'error' : 'default'}
            onClick={() => onRadioChange('front_shock_struts', 'bad')}
          />
          <Chip
            label="Air Suspension"
            clickable
            variant={form.front_shock_struts === 'air_suspension' ? 'filled' : 'outlined'}
            color={form.front_shock_struts === 'air_suspension' ? 'info' : 'default'}
            onClick={() => onRadioChange('front_shock_struts', 'air_suspension')}
          />
          <Chip
            label="Leaking"
            clickable
            variant={form.front_shock_struts === 'leaking' ? 'filled' : 'outlined'}
            color={form.front_shock_struts === 'leaking' ? 'error' : 'default'}
            onClick={() => onRadioChange('front_shock_struts', 'leaking')}
          />
        </Box>
      </Box>
      )}
      ...
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, marginLeft: 'auto' }}>
            ...
            />
            <NotesField
              fieldName="rear_shock_struts"
              fieldLabel="Rear Shock/Struts"
              notes={form.field_notes}
              onNotesChange={onFieldNotesChange}
            />
            <IconButton
              size="small"
              onClick={(e) => onInfoClick(e, "Inspect rear shock absorbers and struts for leaks, damage, or wear.")}
            >
              <InfoIcon fontSize="small" />
            </IconButton>
          </Box>
        ...
        <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          <Chip
            label="✅ Good"
            clickable
            variant={form.rear_shock_struts === 'good' ? 'filled' : 'outlined'}
            color={form.rear_shock_struts === 'good' ? 'success' : 'default'}
            onClick={() => onRadioChange('rear_shock_struts', 'good')}
          />
          <Chip
            label="⚠️ Warning"
            clickable
            variant={form.rear_shock_struts === 'warning' ? 'filled' : 'outlined'}
            color={form.rear_shock_struts === 'warning' ? 'warning' : 'default'}
            onClick={() => onRadioChange('rear_shock_struts', 'warning')}
          />
          <Chip
            label="❌ Bad"
            clickable
            variant={form.rear_shock_struts === 'bad' ? 'filled' : 'outlined'}
            color={form.rear_shock_struts === 'bad' ? 'error' : 'default'}
            onClick={() => onRadioChange('rear_shock_struts', 'bad')}
          />
          <Chip
            label="Air Suspension"
            clickable
            variant={form.rear_shock_struts === 'air_suspension' ? 'filled' : 'outlined'}
            color={form.rear_shock_struts === 'air_suspension' ? 'info' : 'default'}
            onClick={() => onRadioChange('rear_shock_struts', 'air_suspension')}
          />
          <Chip
            label="Leaking"
            clickable
            variant={form.rear_shock_struts === 'leaking' ? 'filled' : 'outlined'}
            color={form.rear_shock_struts === 'leaking' ? 'error' : 'default'}
            onClick={() => onRadioChange('rear_shock_struts', 'leaking')}
          />
        </Box>
      </Box>
      )}
      ...
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, marginLeft: 'auto' }}>
            ...
            <IconButton
              size="small"
              onClick={(e) => onInfoClick(e, "Check for fluid leaks under the vehicle and identify leak type.")}
            >
              <InfoIcon fontSize="small" />
            </IconButton>
          </Box>
        ...
        <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          <Chip
            label="Oil"
            clickable
            variant={form.leaks?.includes('oil') ? 'filled' : 'outlined'}
            color={form.leaks?.includes('oil') ? 'warning' : 'default'}
            onClick={() => {
              const current = form.leaks || '';
              const values = current ? current.split(',') : [];
              const next = values.includes('oil') ? values.filter(v => v !== 'oil') : [...values, 'oil'];
              onRadioChange('leaks', next.join(','));
            }}
          />
          <Chip
            label="Coolant"
            clickable
            variant={form.leaks?.includes('coolant') ? 'filled' : 'outlined'}
            color={form.leaks?.includes('coolant') ? 'info' : 'default'}
            onClick={() => {
              const current = form.leaks || '';
              const values = current ? current.split(',') : [];
              const next = values.includes('coolant') ? values.filter(v => v !== 'coolant') : [...values, 'coolant'];
              onRadioChange('leaks', next.join(','));
            }}
          />
          <Chip
            label="Transmission"
            clickable
            variant={form.leaks?.includes('transmission') ? 'filled' : 'outlined'}
            color={form.leaks?.includes('transmission') ? 'error' : 'default'}
            onClick={() => {
              const current = form.leaks || '';
              const values = current ? current.split(',') : [];
              const next = values.includes('transmission') ? values.filter(v => v !== 'transmission') : [...values, 'transmission'];
              onRadioChange('leaks', next.join(','));
            }}
          />
          <Chip
            label="Differential"
            clickable
            variant={form.leaks?.includes('differential') ? 'filled' : 'outlined'}
            color={form.leaks?.includes('differential') ? 'warning' : 'default'}
            onClick={() => {
              const current = form.leaks || '';
              const values = current ? current.split(',') : [];
              const next = values.includes('differential') ? values.filter(v => v !== 'differential') : [...values, 'differential'];
              onRadioChange('leaks', next.join(','));
            }}
          />
          <Chip
            label="Brake Fluid"
            clickable
            variant={form.leaks?.includes('brake_fluid') ? 'filled' : 'outlined'}
            color={form.leaks?.includes('brake_fluid') ? 'error' : 'default'}
            onClick={() => {
              const current = form.leaks || '';
              const values = current ? current.split(',') : [];
              const next = values.includes('brake_fluid') ? values.filter(v => v !== 'brake_fluid') : [...values, 'brake_fluid'];
              onRadioChange('leaks', next.join(','));
            }}
          />
          <Chip
            ...
            clickable
            variant={form.leaks?.includes('powersteering_fluid') ? 'filled' : 'outlined'}
            color={form.leaks?.includes('powersteering_fluid') ? 'secondary' : 'default'}
            onClick={() => {
              const current = form.leaks || '';
              const values = current ? current.split(',') : [];
              const next = values.includes('powersteering_fluid') ? values.filter(v => v !== 'powersteering_fluid') : [...values, 'powersteering_fluid'];
              onRadioChange('leaks', next.join(','));
            }}
          />
        </Box>
