...
import { logout, getChatConversations, getChatMessages, sendChatMessage, getChatUsers, createOrGetConversation, deleteChatMessage, deleteChatConversation, ChatConversation, ChatMessage, ChatUser, getRoles, UserRole } from '../services/api';
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
    ...
    try {
      ...
      if (isMobile) {
        try {
          ...
          debug.warn('audio', 'Mobile audio unlock failed:', unlockError);
        }
      }
      
      setAudioInitialized(true);
    } catch (error) {
      debug.warn('audio', 'Could not initialize audio:', error);
    }
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const items: { text: string; path: string; icon: JSX.Element }[] = enabled.map(([key, s]: any) => {
          const label = s.title || key;
          const path = key === 'quick_check' ? '/quick-check' : key === 'no_check' ? '/no-check' : key === 'vsi' ? '/vsi' : `/inspect/${key}`;
  ...
  const loadConversations = async () => {
    try {
      ...
      debug.error('chat', 'Failed to load conversations:', error);
    } finally {
  ...
  const loadMessages = async (conversationId: number) => {
    try {
      ...
      debug.error('chat', 'Failed to load messages:', error);
    } finally {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      debug.error('chat', 'Failed to send message:', error);
    } finally {
      setSending(false);
    }
  };
    ...
    try {
      ...
      if (audioContext.state === 'suspended') {
        audioContext.resume().then(() => {
          ...
          debug.warn('audio', 'Could not resume audio context:', error);
      ...
      debug.warn('audio', 'Could not play notification sound:', error);
      // Fallback for mobile - try to play a simple beep
      playFallbackSound();
    }
  };
