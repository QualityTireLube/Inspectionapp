...
export const VirtualTabTimer: React.FC<VirtualTabTimerProps> = ({
  ...
  const changeTab = useCallback((newTabIndex: number) => {
    // Prevent redundant loops if we're already on this index
    if (lastAppliedIndexRef.current === newTabIndex) {
      return;
    }
    ...
    // Record last applied index to avoid effect feedback loops
    lastAppliedIndexRef.current = newTabIndex;
  ...
  useEffect(() => {
    if (!isInitialized.current && tabNames.length > 0) {
      ...
      const timer = setTimeout(() => {
        if (lastAppliedIndexRef.current !== currentTabIndex) {
  ...
  useEffect(() => {
    if (isInitialized.current && tabNames.length > 0) {
      // Only act when parent index differs from our last applied index
      if (lastAppliedIndexRef.current !== currentTabIndex) {
        const currentTabName = tabNames[currentTabIndex];
