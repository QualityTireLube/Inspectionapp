...
import { dk1201Presets } from './dk1201Presets';
...
// DK1201 templates are now imported from dedicated dk1201Presets.ts file

// Convert inches to pixels (assuming 96 DPI)
const inchesToPixels = (inches: number) => Math.round(inches * 96);

// DK1201 templates use their own conversion functions in dk1201Presets.ts

// Paper size for 1.1" x 3.5" labels (Brother QL-800)
const CUSTOM_PAPER_SIZE = {
  width: inchesToPixels(3.5), // 336 pixels
  height: inchesToPixels(1.1)  // 106 pixels
};

// Paper size for 1.8125" x 2.5" labels (Godex 200i)
const GODEX_200I_PAPER_SIZE = {
  width: inchesToPixels(1.8125), // 174 pixels
  height: inchesToPixels(2.5)    // 240 pixels
};

// DK1201 paper dimensions are now handled in dedicated dk1201Presets.ts file
...
export const labelTemplates: LabelTemplate[] = [
  ...
  // GODEX 200I TEMPLATES
  {
    id: uuidv4(),
    labelName: 'Godex 200i Label',
    fields: [
      createField('Created By', 20),
      createField('Created Date', 50),
      createField('Invoice #', 80),
      createField('Product Info', 110),
      createField('Serial/Batch', 140),
      createField('Date Code', 170)
    ],
    paperSize: 'Godex-200i' as any,
    width: GODEX_200I_PAPER_SIZE.width,
    height: GODEX_200I_PAPER_SIZE.height,
    copies: 1,
    archived: false,
    createdBy: 'System',
    createdDate: new Date().toISOString()
  ...
  // BROTHER DK1201 TEMPLATES (29mm wide x 90mm long with horizontal text)
  // Import from dedicated DK1201 preset file
  ...dk1201Presets
];
...
export const getDK1201Templates = (): LabelTemplate[] => {
